...
parser.add_argument('--close_input_face_mouth', default=CLOSE_INPUT_FACE_MOUTH,
                    action='store_true')
...
parser.add_argument('--segment_batch_size', type=int, default=1,
                    help='batch size')
...
predictor = face_alignment.FaceAlignment(face_alignment.LandmarksType._3D,
                                         device='cuda', flip_input=True)
shapes = predictor.get_landmarks(img)
if (not shapes or len(shapes) != 1):
    print('Cannot detect face landmarks. Exit.')
    os.system('rm -f ./examples/*.wav')
    os.system(f'rm -f ./examples/{opt_parser.jpg}')
    exit(-1)
shape_3d = shapes[0]
...
with open(os.path.join('examples', 'dump', 'random_val_gaze.pickle'),
          'wb') as fp:
