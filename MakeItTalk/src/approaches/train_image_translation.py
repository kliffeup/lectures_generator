...
from src.dataset.image_translation.image_translation_dataset import \
    vis_landmark_on_img, vis_landmark_on_img98, vis_landmark_on_img74
...
class Image_translation_block():
    ...
    def __init__(self, opt_parser, single_test=False):
        ...
        if (opt_parser.add_audio_in):
            self.G = ResUnetGenerator(input_nc=7, output_nc=3, num_downs=6,
                                      use_dropout=False)
        else:
            self.G = ResUnetGenerator(input_nc=6, output_nc=3, num_downs=6,
                                      use_dropout=False)
        ...
        if (not single_test):
            ...
            if 'state_dict' not in checkpoint:
                ...
                pretrained_weights = {k: v for k, v in
                                      pretrained_weights.items() \
    ...
    def __train_pass__(self, epoch, is_training=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if (self.opt_parser.comb_fan_awing):
                ...
                fan_pred_landmarks = fan_pred_landmarks.reshape(-1, 68,
                                                                3).detach().cpu().numpy()
            ...
            loss_vgg, loss_style = self.criterionVGG(g_out, image_out,
                                                     style=True)
            ...
            if (self.opt_parser.write):
                self.writer.add_scalar('loss', loss.cpu().detach().numpy(),
                                       self.count)
            ...
            if (i % self.opt_parser.jpg_freq == 0):
                vis_in = np.concatenate([image_in[0,
                ...
                vis_out = np.concatenate(
                    [image_out[0].cpu().detach().numpy().transpose((1, 2, 0)),
                     g_out[0].cpu().detach().numpy().transpose((1, 2, 0))],
                    axis=1)
                vis = np.concatenate([vis_in, vis_out], axis=0)
                try:
                    os.makedirs(os.path.join(self.opt_parser.jpg_dir,
    ...
    def __save_model__(self, save_type, epoch):
        try:
            os.makedirs(
                os.path.join(self.opt_parser.ckpt_dir, self.opt_parser.name))
        except:
            pass
        if (self.opt_parser.write):
            torch.save({
    ...
    def test(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            for j in range(image_in.shape[0] // 16):
                ...
                g_out[g_out < 0] = 0
                ref_out = image_out[
                          j * 16:j * 16 + 16].cpu().detach().numpy().transpose(
                    (0, 2, 3, 1))
                ref_in = image_in[j * 16:j * 16 + 16, 3:6, :,
                         :].cpu().detach().numpy().transpose((0, 2, 3, 1))
                fls_in = image_in[j * 16:j * 16 + 16, 0:3, :,
                         :].cpu().detach().numpy().transpose((0, 2, 3, 1))
            ...
            os.system(
                'ffmpeg -y -i tmp_{:04d}.mp4 -pix_fmt yuv420p random_{:04d}.mp4'.format(
                    i, i))
            os.system('rm tmp_{:04d}.mp4'.format(i))

    def single_test(self, jpg=None, fls=None, filename=None, prefix='',
                    grey_only=False):
        import time
        st = time.time()
        self.G.eval()
        ...
        writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'),
                                 62.5, (256, 256))
        ...
        for i, frame in enumerate(fls):
            ...
            frame = np.concatenate((img_fl, jpg), axis=2).astype(
                np.float32) / 255.0

            image_in, image_out = frame.transpose((2, 0, 1)), np.zeros(
                shape=(3, 256, 256))
            ...
            image_in, image_out = image_in.reshape(-1, 6, 256,
                                                   256), image_out.reshape(-1,
                                                                           3,
                                                                           256,
                                                                           256)
            image_in, image_out = image_in.to(device), image_out.to(device)
            ...
            g_out = g_out.cpu().detach().numpy().transpose((0, 2, 3, 1))
            g_out[g_out < 0] = 0
            ref_in = image_in[:, 3:6, :, :].cpu().detach().numpy().transpose(
                (0, 2, 3, 1))
            fls_in = image_in[:, 0:3, :, :].cpu().detach().numpy().transpose(
                (0, 2, 3, 1))
        ...
        if (filename is None):
            filename = 'v'
        os.system(
            'ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 examples/{}_{}.mp4'.format(
